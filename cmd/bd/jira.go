...
// doPullFromJira imports issues from Jira using the native Go client.
func doPullFromJira(ctx context.Context, dryRun bool, state string) (*PullStats, error) {
	...
	jiraURL, _ := store.GetConfig(ctx, "jira.url")
	jiraProject, _ := store.GetConfig(ctx, "jira.project")
	jiraUsername, _ := store.GetConfig(ctx, "jira.username")
	jiraAPIToken, _ := store.GetConfig(ctx, "jira.api_token")
	...
	if err != nil {
		return stats, fmt.Errorf("failed to create Jira client: %w", err)
	}

	// Fetch issues from Jira
	jiraIssues, err := client.SearchIssues(ctx, "", state)
	if err != nil {
	...
	for k, v := range jira.DefaultStatusMapping {
		statusMap[k] = v
	}
	// TODO: Load custom mappings from jira.status_map.* config keys

	// Load custom type mappings
	typeMap := make(map[string]types.IssueType)
	for k, v := range jira.DefaultTypeMapping {
		typeMap[k] = v
	}
	// TODO: Load custom mappings from jira.type_map.* config keys

	// Load custom priority mappings
	priorityMap := make(map[string]int)
	for k, v := range jira.DefaultPriorityMapping {
		priorityMap[k] = v
	}
	// TODO: Load custom mappings from jira.priority_map.* config keys

	// Convert Jira issues to bd issues
	// Note: IDs will be generated by the import logic, not pre-generated here
	converter := jira.NewConverter(jira.ConverterConfig{
		JiraURL:     jiraURL,
		StatusMap:   statusMap,
		TypeMap:     typeMap,
		PriorityMap: priorityMap,
		// IDGenerator is nil - let import logic generate IDs
	})

	issues, err := converter.Convert(jiraIssues)
	if err != nil {
		return stats, fmt.Errorf("failed to convert issues: %w", err)
	}
...
// NOTE: This is only used for --push functionality. Pull uses native Go.
func findJiraScript(name string) (string, error) {
	...
	return "", fmt.Errorf("script not found: %s\n"+
		"Push functionality requires the Python script from the beads repository.\n"+
		"Download from: https://github.com/steveyegge/beads/blob/main/examples/jira-import/%s\n"+
		"Place in: examples/jira-import/ relative to your working directory",
		name, name)
}
